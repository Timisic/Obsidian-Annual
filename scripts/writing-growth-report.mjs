...
function main() {
  ...
  const outDir = resolve(
    args.out ?? config.out ?? join(vault, "Annual Reviews", "Writing Growth"),
  );
  const excludes = listValue(args.exclude ?? config.exclude, DEFAULT_EXCLUDES);
  const threshold = Number(
    args.threshold ?? config.writingDayThreshold ?? DEFAULT_THRESHOLD,
  );
  const history = String(args.history ?? config.history ?? "auto");
  const snapshotFile = resolve(
    args.snapshotFile ??
      config.snapshotFile ??
      join(outDir, "writing-growth-snapshots.json"),
  );
  ...
  const gitSnapshots =
    history === "git" || history === "auto"
      ? readGitSnapshots(vault, excludes, period)
      : [];
  const snapshots = mergeSnapshots(
    history === "none" ? [] : [...storedSnapshots, ...gitSnapshots],
    currentSnapshot,
  );
  ...
  const prefix =
    period.type === "month" ? period.startDate.slice(0, 7) : period.startDate.slice(0, 4);
  ...
  const markdown = renderMarkdown(
    report.summary,
    report.feedback,
    [growthPath, monthlyPath, heatmapPath].map((path) =>
      relative(vault, path).split("/").join("/"),
    ),
  );
...
function printHelp() {
  process.stdout
    .write(`Usage: node scripts/writing-growth-report.mjs --vault <path> --year 2026 [options]
...
function resolvePeriod(args, config) {
  ...
  if ((args.start ?? config.startDate) && (args.end ?? config.endDate)) {
    return {
      type: "custom",
      startDate: String(args.start ?? config.startDate),
      endDate: String(args.end ?? config.endDate),
    };
  }
...
function readGitSnapshots(vault, excludes, period) {
  try {
    execFileSync("git", ["-C", vault, "rev-parse", "--show-toplevel"], {
      stdio: "ignore",
    });
  } catch {
    return [];
  ...
  const log = execFileSync(
    "git",
    ["-C", vault, "log", "--reverse", "--date=short", "--format=%H%x09%cs", "--", "*.md"],
    { encoding: "utf8" },
  ).trim();
  if (!log) return [];
  ...
  const baseline = [...byDate.entries()]
    .filter(([date]) => date < period.startDate)
    .pop();
  const selected = [...byDate.entries()].filter(
...
function gitSnapshot(vault, excludes, date, hash) {
  ...
  const names = execFileSync(
    "git",
    ["-C", vault, "ls-tree", "-r", "--name-only", hash, "--", "*.md"],
    { encoding: "utf8" },
  )
    .split(/\r?\n/)
    .filter(Boolean)
  ...
  for (const path of names) {
    try {
      const content = execFileSync("git", ["-C", vault, "show", `${hash}:${path}`], {
        encoding: "utf8",
...
function mergeSnapshots(snapshots, currentSnapshot) {
  ...
  for (const snapshot of [...snapshots, currentSnapshot]) {
    if (/^\d{4}-\d{2}-\d{2}$/.test(snapshot.date))
      byDate.set(snapshot.date, normalizeSnapshot(snapshot));
  }
...
function normalizeSnapshot(snapshot) {
  return {
    ...
    files: Object.fromEntries(
      Object.entries(snapshot.files ?? {}).map(([path, value]) => [
        path,
        { words: typeof value === "number" ? value : Number(value.words) || 0 },
      ]),
    ),
  };
}
...
function buildReport(snapshots, period, threshold) {
  ...
  const baselineOnly =
    normalized.length <= 1 ||
    (inPeriod.length <= 1 &&
      !normalized.some((snapshot) => snapshot.date < period.startDate));
  const summary = {
    ...
    peak_month:
      monthly
        .filter((month) => month.addedWords > 0)
        .sort((a, b) => b.addedWords - a.addedWords || a.month.localeCompare(b.month))[0]
        ?.month ?? null,
    top_days: topDays,
...
function feedback(summary, days) {
  ...
  return {
    strength:
      ratio >= 0.45 || summary.longest_streak >= 14
...
function renderMarkdown(summary, reportFeedback, chartPaths) {
  return [
    ...
    summary.baseline_only
      ? summary.baseline_message
      : `本期新增字数 ${summary.total_added_words.toLocaleString("en-US")}，写作天数 ${summary.writing_days} 天，最长连续写作 ${summary.longest_streak} 天。`,
    "",
...
function renderDailySvg(daily) {
  const points = daily
    .map(
      (day, index) =>
        `${index * 8},${220 - Math.min(200, (day.cumulativeWords / Math.max(1, daily.at(-1)?.cumulativeWords ?? 1)) * 200)}`,
    )
    .join(" ");
...
function renderMonthlySvg(monthly) {
  ...
  const bars = monthly
    .map(
      (month, index) =>
        `<rect x="${20 + index * 36}" y="${220 - (month.addedWords / max) * 190}" width="24" height="${(month.addedWords / max) * 190}" rx="3" fill="#b95e43"><title>${month.month}: ${month.addedWords}</title></rect>`,
    )
    .join("");
...
function enumerateDates(start, end) {
  ...
  for (
    ...
    current <= new Date(`${end}T00:00:00`);
...
function heatColor(words, max) {
  ...
  return colors[
    Math.min(colors.length - 1, Math.ceil((words / max) * colors.length) - 1)
  ];
}
