...
当前 MVP 的 canonical scope 是 AI-assisted Time Range Review 主闭环：ReviewSession → 受控 Evidence Package → AI semantic Theme Hypotheses → Review Board 决策 → evidence-backed Markdown report。README 面向普通用户上手，SPEC 说明目标行为和验证要求；如果 Feature Scope / Feature Inventory 的历史表述与这个闭环冲突，先按当前 SPEC 与 Review Board 规格收敛主路径，再补充清理历史文档。
...
- [AI 报告生成设计](ai-report-design.md)：历史设计记录；当前主路径以受控 evidence package 驱动 AI semantic themes，并必须经过 Review Board 复核。
...
- AI 是当前主题分析主路径，但必须受 evidence package、范围过滤和人工复核约束；Dashboard、图表、导出和 provider 扩展只能在明确标注为可选或 backlog 时出现。
