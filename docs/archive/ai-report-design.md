...
关联规格：[产品规格](product-specification.md)
...
- 一句话判断：基于写作增长、主题演化和已确认主题假设生成 1 句可编辑总结。
- 写作增长表达：确定性报告提供总新增字数、写作天数、最长连续写作、日累计字数图、月度增长图、热力图和 1 条优点/风险/建议。
- 主题演化表达：确定性报告提供 Top 5-8 主题、主题演化图、新兴主题、衰退主题和 1 条下期建议。
- 主题假设表达：确定性报告只写入用户确认后的 Theme Hypotheses 和证据链接。
- 复盘提示表达：报告可以提供可选 reflection prompts，但不强制生成下一步行动项。
...
- `## 主题假设`：固定包含 Review Board 中已确认的 Theme Hypotheses 和证据链接。
- `## 复盘提示`：只提供可选复盘提示；不要求用户创建行动项。
...
- `buildYearAggregate` 聚合写作增长、主题演化、证据笔记、孤立潜力笔记和支持 Theme Hypothesis 预览的必要信号。
...
| path clusters         | 文件夹、日记路径、项目路径                                   | 作为弱证据信号帮助解释 Evidence Cluster，不直接生成项目/任务卡片。 |
