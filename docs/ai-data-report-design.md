...
年度报告适合让 AI 生成长周期叙事，但不适合让 AI 重新计算指标。插件先生成确定性数据，再把数据交给 AI 做表达层增强：

- 年度主题总结：从高频标签、文件夹、链接和代表笔记中提炼 3 到 5 个主题。
- 年度节奏解读：解释月度峰值、低谷、连续记录和写作量变化。
- 年度关系洞察：指出被反复引用的笔记、主题桥接笔记、可能的 MOC 或项目主线。
- 代表性证据串联：把多篇源笔记组织成“今年我如何推进某个主题”的段落。
- 下一年行动建议：基于未完成任务、反复出现的主题和低连接笔记生成可执行建议。
- 可分享摘要：生成一段用户可二次编辑的公开摘要，但默认不导出、不发布。
...
AI 生成内容应以 Markdown 片段进入最终报告，而不是独立的不可编辑页面：

- `## AI Personalization`：当前实现已有的个性化段落入口。
- `## 年度主题` / `## 月度主题`：带证据链接的主题归纳。
- `## 值得回看的笔记`：按主题或月份组织的源笔记列表。
- `## 行动建议`：短列表，每项都引用来源或统计依据。
- `## 数据不足提示`：当样本少、摘录被省略或隐私设置限制上下文时，明确告诉用户。
...
- `buildYearAggregate` 聚合年度/月度指标、热门标签、热门文件夹、热门链接和代表笔记。
